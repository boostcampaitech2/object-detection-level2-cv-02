...
parser.add_argument('--annotations', default = '/train_original.json', 
                    type=str, help='Path to COCO annotations file.')
parser.add_argument('--save_root', default = '/opt/ml/detection/object-detection-level2-cv-02/template/mmdetection/tools', 
                    type=str,help='Path to data root')
parser.add_argument('--data_root', default = '/opt/ml/detection/dataset', type=str,
                    help='Path to data root')
parser.add_argument('--train', type=str, default = f'/train_fold{0}.json', help='Where to store COCO training annotations')
parser.add_argument('--test', type=str, default = f'/val_fold{0}.json', help='Where to store COCO test annotations')
parser.add_argument('--split', dest='split', default = 0.9,  type=float,
                    help="A percentage of a split; a number in (0, 1)")
parser.add_argument('--having-annotations', default = True, dest='having_annotations', action='store_true',
...
# k-fold
def stratified_group_k_fold(X, y, groups, k, seed = 123456):
    ...
    y_counts_per_group = X.groupby(['image_id', 'category_id']).size().unstack(fill_value=0) # shape = (n_images, n_label)
...
def main(args, dev_ind, val_ind):
    with open(args.data_root+args.annotations, 'rt', encoding='UTF-8') as annotations:
        ...
        print("Saved {} entries in {} and {} in {}".format(len(x), args.data_root+args.train, len(y), args.data_root+args.val))
