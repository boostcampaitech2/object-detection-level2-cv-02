...
def convert(src, dst, depth):
    ...
    for i in range(1, len(block_nums) + 1):
        for j in range(block_nums[i - 1]):
            if j == 0:
                convert_conv_fc(
                    blobs, state_dict, f"res{i + 1}_{j}_branch1", f"layer{i}.{j}.downsample.0", converted_names
                )
                convert_bn(
                    blobs, state_dict, f"res{i + 1}_{j}_branch1_bn", f"layer{i}.{j}.downsample.1", converted_names
                )
            for k, letter in enumerate(["a", "b", "c"]):
                convert_conv_fc(
                    blobs, state_dict, f"res{i + 1}_{j}_branch2{letter}", f"layer{i}.{j}.conv{k+1}", converted_names
                )
                convert_bn(
                    blobs, state_dict, f"res{i + 1}_{j}_branch2{letter}_bn", f"layer{i}.{j}.bn{k + 1}", converted_names
                )
