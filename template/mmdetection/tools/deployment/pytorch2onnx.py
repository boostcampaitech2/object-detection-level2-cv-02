...
def pytorch2onnx(
    ...
    if skip_postprocess:
        ...
        print(f"Successfully exported ONNX model without " f"post process: {output_file}")
        return
    ...
    if do_simplify:
        ...
        model_opt, check_ok = onnxsim.simplify(
            output_file, input_data=input_dic, custom_lib=ort_custom_op_path, dynamic_input_shape=dynamic_export
        )
        if check_ok:
    ...
    if verify:
        ...
        model.show_result(
            show_img, pytorch_results, score_thr=score_thr, show=True, win_name="PyTorch", out_file=out_file_pt
        )
        onnx_model.show_result(
            show_img, onnx_results, score_thr=score_thr, show=True, win_name="ONNXRuntime", out_file=out_file_ort
        )
