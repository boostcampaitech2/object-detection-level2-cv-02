...
def onnx2tensorrt(onnx_file, trt_file, input_config, verify=False, show=False, workspace_size=1, verbose=False):
    ...
    if verify:
        ...
        trt_model.show_result(
            show_img, trt_results, score_thr=score_thr, show=True, win_name="TensorRT", out_file=out_file_trt
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--min-shape", type=int, nargs="+", default=None, help="Minimum input size of the model in TensorRT"
    )
    parser.add_argument(
        "--max-shape", type=int, nargs="+", default=None, help="Maximum input size of the model in TensorRT"
    )
