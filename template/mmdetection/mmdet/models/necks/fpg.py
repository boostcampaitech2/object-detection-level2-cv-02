...
class FPG(BaseModule):
    ...
    def __init__(
        ...
        init_cfg=[
            ...
            dict(type="Constant", layer=["_BatchNorm", "_InstanceNorm", "GroupNorm", "LayerNorm"], val=1.0),
        ],
    ):
        ...
        for i in range(extra_levels):
            if self.add_extra_convs:
                ...
                extra_conv = nn.Conv2d(
                    self.inter_channels[fpn_idx - 1], self.inter_channels[fpn_idx], 3, stride=2, padding=1
                )
        ...
        for i in range(self.num_outs):
            trans = self.build_trans(
                self.output_trans, self.inter_channels[i], self.out_channels, num_inputs=self.stack_times + 1
            )
