...
class SeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, bias=False, relu=False):
        super(SeparableConv, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.relu = relu

        self.sep = nn.Conv2d(in_channels, in_channels, 3, padding=1, groups=in_channels, bias=False)
...
class WeightedInputConv(nn.Module):
    def __init__(
        ...
        self.pw_conv = ConvModule(
            in_channels, out_channels, 1, norm_cfg=norm_cfg, act_cfg=dict(type="ReLU"), inplace=False
        )
...
class WeightedInputConv_V2(nn.Module):
    def __init__(
        ...
        if separable_conv:
            ...
            self.conv_op = nn.Sequential(SeparableConv(in_channels, out_channels, bias=True, relu=False), bn_layer)
        else:
            self.conv_op = ConvModule(
                in_channels, out_channels, 3, padding=1, conv_cfg=None, norm_cfg=norm_cfg, act_cfg=None, inplace=False
            )
...
class ResampingConv(nn.Module):
    ...
    def __init__(
        ...
        if self.in_stride < self.out_stride:
            ...
            assert scale == 2
            self.rescale_op = nn.MaxPool2d(scale + 1, stride=scale, padding=1)
            # self.rescale_op = nn.MaxPool2d(2, stride=scale)
        else:
            if self.in_stride > self.out_stride:
                scale = self.in_stride // self.out_stride
                self.rescale_op = functools.partial(F.interpolate, scale_factor=scale, mode="nearest")
            else:
        ...
        if self.in_channels != self.out_channels:
            if separable_conv:
                ...
                self.conv_op = ConvModule(in_channels, out_channels, 1, norm_cfg=norm_cfg, act_cfg=None, inplace=False)
...
class bifpn(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for fnode, op_node, new_op_node in zip(self.nodes_settings, self.resample_op_nodes, self.new_op_nodes):
            input_node = []
        ...
        if extra_levels >= 1:
            ...
            for _ in range(extra_levels):
                self.extra_convs.append(
                    ResampingConv(
                        bifpn_in_channels[-1], bifpn_strides[-1], bifpn_strides[-1] * 2, out_channels, norm_cfg=norm_cfg
                    )
                )
