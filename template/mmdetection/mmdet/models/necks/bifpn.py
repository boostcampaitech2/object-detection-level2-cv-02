...
class BiFPNModule(nn.Module):
    def __init__(self, channels, levels, init=0.5, conv_cfg=None, norm_cfg=None, act_cfg=dict(type="relu")):
        ...
        for jj in range(2):
            for i in range(self.levels - 1):  # 1,2,3
                fpn_conv = nn.Sequential(
                    ...
                    ConvModule(
                        channels, channels, 1, conv_cfg=conv_cfg, norm_cfg=norm_cfg, act_cfg=self.act_cfg, inplace=False
                    ),
                )
    ...
    def forward(self, inputs):
        ...
        pathtd[levels - 1] = w1[0, kk] * pathtd[levels - 1] + w1[1, kk] * F.max_pool2d(
            pathtd[levels - 2], kernel_size=2
        )
