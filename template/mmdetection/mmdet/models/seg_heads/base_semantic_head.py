...
class BaseSemanticHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(
        self, num_classes, init_cfg=None, loss_seg=dict(type="CrossEntropyLoss", ignore_index=255, loss_weight=1.0)
    ):
    ...
    def loss(self, seg_preds, gt_semantic_seg):
        ...
        loss_seg = self.loss_seg(
            seg_preds.reshape(-1, self.num_classes), gt_semantic_seg.reshape(-1).long()  # => [NxHxW, C]
        )
