...
class PanopticFPNHead(BaseSemanticHead):
    ...
    def _set_things_to_void(self, gt_semantic_seg):
        ...
        fg_mask = (gt_semantic_seg >= self.fg_range[0]) * (gt_semantic_seg <= self.fg_range[1])
        bg_mask = (gt_semantic_seg >= self.bg_range[0]) * (gt_semantic_seg <= self.bg_range[1])

        new_gt_seg = torch.clone(gt_semantic_seg)
        new_gt_seg = torch.where(bg_mask, gt_semantic_seg - self.fg_nums, new_gt_seg)
        new_gt_seg = torch.where(fg_mask, fg_mask.int() * self.bg_nums, new_gt_seg)
        return new_gt_seg
