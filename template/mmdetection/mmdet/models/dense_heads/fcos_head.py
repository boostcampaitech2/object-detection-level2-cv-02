...
class FCOSHead(AnchorFreeHead):
    ...
    def _get_bboxes(
        ...
        for cls_score, bbox_pred, centerness, points in zip(cls_scores, bbox_preds, centernesses, mlvl_points):
            ...
            if nms_pre > 0:
                ...
                if torch.onnx.is_in_onnx_export():
                    ...
                    scores = scores.reshape(-1, self.num_classes)[transformed_inds, :].reshape(
                        batch_size, -1, self.num_classes
                    )
                    centerness = centerness.reshape(-1, 1)[transformed_inds].reshape(batch_size, -1)
                else:
        ...
        if torch.onnx.is_in_onnx_export() and with_nms:
            ...
            batch_mlvl_scores = batch_mlvl_scores * (batch_mlvl_centerness.unsqueeze(2))
        ...
        if with_nms:
            ...
            for (mlvl_bboxes, mlvl_scores, mlvl_centerness) in zip(
                ...
                det_bbox, det_label = multiclass_nms(
                    mlvl_bboxes, mlvl_scores, cfg.score_thr, cfg.nms, cfg.max_per_img, score_factors=mlvl_centerness
                )
                det_results.append(tuple([det_bbox, det_label]))
        else:
            det_results = [
                tuple(mlvl_bs) for mlvl_bs in zip(batch_mlvl_bboxes, batch_mlvl_scores, batch_mlvl_centerness)
            ]
        return det_results
