        ...
        if self.force_topk:
            ...
            if gt_inds_no_points_inside.numel():
                ...
                inside_gt_bbox_mask[:, gt_inds_no_points_inside] = torch.scatter(
                    temp_mask, dim=0, index=topk_center_index, src=torch.ones_like(topk_center_index, dtype=torch.bool)
                )
...
class AutoAssignHead(FCOSHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.center_prior = CenterPrior(
            force_topk=force_topk, topk=topk, num_classes=self.num_classes, strides=self.strides
        )
    ...
    def loss(self, cls_scores, bbox_preds, objectnesses, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None):
        ...
        (pos_loss_list,) = multi_apply(
            self.get_pos_loss_single, cls_scores, objectnesses, reg_loss_list, gt_labels, center_prior_weight_list
        )
        ...
        (neg_loss_list,) = multi_apply(
            self.get_neg_loss_single, cls_scores, objectnesses, gt_labels, ious_list, inside_gt_bbox_mask_list
        )
