...
class SOLOHead(BaseMaskHead):
    ...
    def __init__(
        ...
        init_cfg=[
            ...
            dict(type="Normal", std=0.01, bias_prob=0.01, override=dict(name="conv_mask_list")),
    ...
    def loss(self, mlvl_mask_preds, mlvl_cls_preds, gt_labels, gt_masks, img_metas, gt_bboxes=None, **kwargs):
        ...
        pos_mask_targets, labels, pos_masks = multi_apply(
            self._get_targets_single, gt_bboxes, gt_labels, gt_masks, featmap_sizes=featmap_sizes
        )
    ...
    def _get_targets_single(self, gt_bboxes, gt_labels, gt_masks, featmap_sizes=None):
        ...
        for (lower_bound, upper_bound), stride, featmap_size, num_grid in zip(
            ...
            mask_target = torch.zeros(
                [num_grid ** 2, featmap_size[0], featmap_size[1]], dtype=torch.uint8, device=device
            )
            ...
            for gt_mask, gt_label, pos_h_range, pos_w_range, valid_mask_flag in zip(
                hit_gt_masks, hit_gt_labels, pos_h_ranges, pos_w_ranges, valid_mask_flags
            ):
                ...
                # left, top, right, down
                top_box = max(0, int(((center_h - pos_h_range) / upsampled_size[0]) // (1.0 / num_grid)))
                down_box = min(num_grid - 1, int(((center_h + pos_h_range) / upsampled_size[0]) // (1.0 / num_grid)))
                left_box = max(0, int(((center_w - pos_w_range) / upsampled_size[1]) // (1.0 / num_grid)))
                right_box = min(num_grid - 1, int(((center_w + pos_w_range) / upsampled_size[1]) // (1.0 / num_grid)))
...
class DecoupledSOLOHead(SOLOHead):
    ...
    def __init__(
        ...
        *args,
        init_cfg=[
            dict(type="Normal", layer="Conv2d", std=0.01),
            dict(type="Normal", std=0.01, bias_prob=0.01, override=dict(name="conv_mask_list_x")),
            dict(type="Normal", std=0.01, bias_prob=0.01, override=dict(name="conv_mask_list_y")),
            dict(type="Normal", std=0.01, bias_prob=0.01, override=dict(name="conv_cls")),
        ],
        **kwargs,
    ):
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):
            ...
            self.mask_convs_x.append(
                ConvModule(chn, self.feat_channels, 3, stride=1, padding=1, norm_cfg=self.norm_cfg)
            )
    ...
    def get_results(self, mlvl_mask_preds_x, mlvl_mask_preds_y, mlvl_cls_scores, img_metas, rescale=None, **kwargs):
        ...
        for img_id in range(len(img_metas)):
            ...
            results = self._get_results_single(
                cls_pred_list, mask_pred_list_x, mask_pred_list_y, img_meta=img_metas[img_id], cfg=self.test_cfg
            )
            results_list.append(results)
        return results_list
