...
class FeatureAdaption(BaseModule):
    ...
    def __init__(
        ...
        deform_groups=4,
        init_cfg=dict(
            type="Normal", layer="Conv2d", std=0.1, override=dict(type="Normal", name="conv_adaption", std=0.01)
        ),
    ):
...
class GuidedAnchorHead(AnchorHead):
            ...
            for i, flags in enumerate(multi_level_approx_flags):
                ...
                for i in range(self.approxs_per_octave):
                    ...
                    inside_flags = anchor_inside_flags(
                        split_approxs, split_valid_flags, img_meta["img_shape"][:2], self.train_cfg.allowed_border
                    )
    ...
    def _ga_shape_target_single(
        self, flat_approxs, inside_flags, flat_squares, gt_bboxes, gt_bboxes_ignore, img_meta, unmap_outputs=True
    ):
    ...
    def ga_shape_targets(
        ...
        (all_bbox_anchors, all_bbox_gts, all_bbox_weights, pos_inds_list, neg_inds_list) = multi_apply(
    ...
    def loss_loc_single(self, loc_pred, loc_target, loc_weight, loc_avg_factor):
        loss_loc = self.loss_loc(
            loc_pred.reshape(-1, 1), loc_target.reshape(-1).long(), loc_weight.reshape(-1), avg_factor=loc_avg_factor
        )
        return loss_loc

    @force_fp32(apply_to=("cls_scores", "bbox_preds", "shape_preds", "loc_preds"))
    def loss(
        self, cls_scores, bbox_preds, shape_preds, loc_preds, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None
    ):
    ...
    def get_bboxes(self, cls_scores, bbox_preds, shape_preds, loc_preds, img_metas, cfg=None, rescale=False):
        ...
        _, guided_anchors, loc_masks = self.get_anchors(
            featmap_sizes, shape_preds, loc_preds, img_metas, use_loc_filter=not self.training, device=device
        )
        result_list = []
        for img_id in range(len(img_metas)):
            ...
            proposals = self._get_bboxes_single(
                cls_score_list, bbox_pred_list, guided_anchor_list, loc_mask_list, img_shape, scale_factor, cfg, rescale
            )
            result_list.append(proposals)
        return result_list

    def _get_bboxes_single(
        self, cls_scores, bbox_preds, mlvl_anchors, mlvl_masks, img_shape, scale_factor, cfg, rescale=False
    ):
