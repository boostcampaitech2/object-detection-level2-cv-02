...
from mmdet.core import build_assigner, build_sampler, images_to_levels, multi_apply, multiclass_nms, unmap
...
class RepPointsHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.point_generator = MlvlPointGenerator(self.point_strides, offset=0.0)
    ...
    def points2bbox(self, pts, y_first=True):
        ...
        if self.transform_method == "minmax":
            ...
            bbox = torch.cat(
                [pts_x_mean - half_width, pts_y_mean - half_height, pts_x_mean + half_width, pts_y_mean + half_height],
                dim=1,
            )
        else:
    ...
    def get_points(self, featmap_sizes, img_metas, device):
        ...
        multi_level_points = self.point_generator.grid_priors(featmap_sizes, device, with_stride=True)
        points_list = [[point.clone() for point in multi_level_points] for _ in range(num_imgs)]

        # for each image, we compute valid flags of multi level grids
        valid_flag_list = []
        for img_id, img_meta in enumerate(img_metas):
            multi_level_flags = self.point_generator.valid_flags(featmap_sizes, img_meta["pad_shape"])
    ...
    def _point_target_single(
        self, flat_proposals, valid_flags, gt_bboxes, gt_bboxes_ignore, gt_labels, stage="init", unmap_outputs=True
    ):
    ...
    def loss(
        self, cls_scores, pts_preds_init, pts_preds_refine, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None
    ):
    ...
    def get_bboxes(
        self, cls_scores, pts_preds_init, pts_preds_refine, img_metas, cfg=None, rescale=False, with_nms=True
    ):
        assert len(cls_scores) == len(pts_preds_refine)
        device = cls_scores[0].device
        bbox_preds_refine = [self.points2bbox(pts_pred_refine) for pts_pred_refine in pts_preds_refine]
        num_levels = len(cls_scores)
        featmap_sizes = [cls_scores[i].size()[-2:] for i in range(len(cls_scores))]
        multi_level_points = self.point_generator.grid_priors(featmap_sizes, device)

        result_list = []
        for img_id in range(len(img_metas)):
            cls_score_list = [cls_scores[i][img_id] for i in range(num_levels)]
            bbox_pred_list = [bbox_preds_refine[i][img_id] for i in range(num_levels)]
            img_shape = img_metas[img_id]["img_shape"]
            scale_factor = img_metas[img_id]["scale_factor"]
            proposals = self._get_bboxes_single(
                cls_score_list, bbox_pred_list, multi_level_points, img_shape, scale_factor, cfg, rescale, with_nms
            )
            result_list.append(proposals)
        return result_list

    def _get_bboxes_single(
        self, cls_scores, bbox_preds, mlvl_points, img_shape, scale_factor, cfg, rescale=False, with_nms=True
    ):
