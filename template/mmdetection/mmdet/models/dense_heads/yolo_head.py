...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
        ...
        if with_nms and cfg.get("nms", None) is not None:
            ...
            for (mlvl_bboxes, mlvl_scores, mlvl_conf_scores) in zip(
                ...
                det_bboxes, det_labels = multiclass_nms(
                    mlvl_bboxes, mlvl_scores, cfg.score_thr, cfg.nms, cfg.max_per_img, score_factors=mlvl_conf_scores
                )
                det_results.append(tuple([det_bboxes, det_labels]))

        else:
            det_results = [
                tuple(mlvl_bs) for mlvl_bs in zip(batch_mlvl_bboxes, batch_mlvl_scores, batch_mlvl_conf_scores)
            ]
        return det_results
    ...
    def get_targets(self, anchor_list, responsible_flag_list, gt_bboxes_list, gt_labels_list):
        ...
        results = multi_apply(
            self._get_targets_single, anchor_list, responsible_flag_list, gt_bboxes_list, gt_labels_list
        )
