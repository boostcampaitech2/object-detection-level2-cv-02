...
class YOLOFHead(AnchorHead):
    ...
    def _init_layers(self):
        ...
        self.cls_score = nn.Conv2d(
            self.in_channels, self.num_anchors * self.num_classes, kernel_size=3, stride=1, padding=1
        )
    ...
    def loss(self, cls_scores, bbox_preds, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None):
        ...
        if batch_pos_predicted_boxes.shape[0] == 0:
            ...
            loss_bbox = self.loss_bbox(
                batch_pos_predicted_boxes, batch_target_boxes, batch_bbox_weights.float(), avg_factor=num_total_samples
            )
    ...
    def _get_targets_single(
        ...
        assign_result = self.assigner.assign(
            decoder_bbox_preds, anchors, gt_bboxes, gt_bboxes_ignore, None if self.sampling else gt_labels
        )
