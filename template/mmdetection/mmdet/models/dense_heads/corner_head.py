...
from ..utils.gaussian_target import gather_feat, get_local_maximum, get_topk_from_heatmap, transpose_and_gather_feat
...
class CornerHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _init_corner_kpt_layers(self):
        ...
        for _ in range(self.num_feat_levels):
            ...
            self.tl_off.append(
                self._make_layers(out_channels=self.corner_offset_channels, in_channels=self.in_channels)
            )
            self.br_off.append(
                self._make_layers(out_channels=self.corner_offset_channels, in_channels=self.in_channels)
            )
    ...
    def get_targets(
        ...
        if with_guiding_shift:
            target_result.update(
                topleft_guiding_shift=gt_tl_guiding_shift, bottomright_guiding_shift=gt_br_guiding_shift
            )
        if with_centripetal_shift:
            target_result.update(
                topleft_centripetal_shift=gt_tl_centripetal_shift, bottomright_centripetal_shift=gt_br_centripetal_shift
            )
    ...
    def loss(
        ...
        targets = self.get_targets(
            gt_bboxes, gt_labels, tl_heats[-1].shape, img_metas[0]["pad_shape"], with_corner_emb=self.with_corner_emb
        )
    ...
    def get_bboxes(
        self, tl_heats, br_heats, tl_embs, br_embs, tl_offs, br_offs, img_metas, rescale=False, with_nms=True
    ):
