...
class YOLACTHead(AnchorHead):
    ...
    def _init_layers(self):
        ...
        self.conv_cls = nn.Conv2d(self.feat_channels, self.num_anchors * self.cls_out_channels, 3, padding=1)
        self.conv_reg = nn.Conv2d(self.feat_channels, self.num_anchors * 4, 3, padding=1)
        self.conv_coeff = nn.Conv2d(self.feat_channels, self.num_anchors * self.num_protos, 3, padding=1)
    ...
    def get_bboxes(self, cls_scores, bbox_preds, coeff_preds, img_metas, cfg=None, rescale=False):
        ...
        for img_id in range(len(img_metas)):
            ...
            bbox_res = self._get_bboxes_single(
                cls_score_list, bbox_pred_list, coeff_pred_list, mlvl_anchors, img_shape, scale_factor, cfg, rescale
            )
    ...
    def _get_bboxes_single(
        ...
        det_bboxes, det_labels, det_coeffs = fast_nms(
            mlvl_bboxes, mlvl_scores, mlvl_coeffs, cfg.score_thr, cfg.iou_thr, cfg.top_k, cfg.max_per_img
        )
        return det_bboxes, det_labels, det_coeffs
...
class YOLACTProtonet(BaseModule):
        ...
        for num_channels, kernel_size in zip(self.proto_channels, self.proto_kernel_sizes):
            if kernel_size > 0:
                ...
                if num_channels is None:
                    ...
                    layer = nn.ConvTranspose2d(in_channels, num_channels, -kernel_size, padding=kernel_size // 2)
    ...
    def get_targets(self, mask_pred, gt_masks, pos_assigned_gt_inds):
        ...
        gt_masks = F.interpolate(gt_masks.unsqueeze(0), (mask_h, mask_w), mode="bilinear", align_corners=False).squeeze(
            0
        )
