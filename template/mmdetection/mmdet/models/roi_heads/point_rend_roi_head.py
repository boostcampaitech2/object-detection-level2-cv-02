    ...
    def _mask_point_forward_test(self, x, rois, label_pred, mask_pred, img_metas):
        ...
        for subdivision_step in range(self.test_cfg.subdivision_steps):
            refined_mask_pred = F.interpolate(
                refined_mask_pred, scale_factor=self.test_cfg.scale_factor, mode="bilinear", align_corners=False
            )
            ...
            for i in range(num_imgs):
                if det_bboxes[i].shape[0] == 0:
                    ...
                    segm_result = self.mask_head.get_seg_masks(
                        mask_pred_i, _bboxes[i], det_labels[i], self.test_cfg, ori_shapes[i], scale_factors[i], rescale
                    )
