...
from mmdet.core import bbox2result, bbox2roi, bbox_mapping, merge_aug_bboxes, merge_aug_masks, multiclass_nms
...
class SCNetRoIHead(CascadeRoIHead):
    ...
    def _bbox_forward_train(
        self, stage, x, sampling_results, gt_bboxes, gt_labels, rcnn_train_cfg, semantic_feat=None, glbctx_feat=None
    ):
    ...
    def _mask_forward_train(
        self, x, sampling_results, gt_masks, rcnn_train_cfg, semantic_feat=None, glbctx_feat=None, relayed_feat=None
    ):
        """Run forward function and calculate loss for mask head in
        training."""
        pos_rois = bbox2roi([res.pos_bboxes for res in sampling_results])
        mask_results = self._mask_forward(
            x, pos_rois, semantic_feat=semantic_feat, glbctx_feat=glbctx_feat, relayed_feat=relayed_feat
        )
    ...
    def forward_train(
        ...
        for i in range(self.num_stages):
            ...
            bbox_results = self._bbox_forward_train(
                i, x, sampling_results, gt_bboxes, gt_labels, rcnn_train_cfg, semantic_feat, glbctx_feat
            )
            ...
            if i < self.num_stages - 1:
                ...
                with torch.no_grad():
                    proposal_list = self.bbox_head[i].refine_bboxes(
                        bbox_results["rois"], roi_labels, bbox_results["bbox_pred"], pos_is_gts, img_metas
                    )
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
            ...
            if i < self.num_stages - 1:
                ...
                for j in range(num_imgs):
                    if rois[j].shape[0] > 0:
                        ...
                        refine_rois = bbox_head.regress_by_class(rois[j], bbox_label[j], bbox_pred[j], img_metas[j])
        ...
        for i in range(num_imgs):
            det_bbox, det_label = self.bbox_head[-1].get_bboxes(
                rois[i], cls_score[i], bbox_pred[i], img_shapes[i], scale_factors[i], rescale=rescale, cfg=rcnn_test_cfg
            )
        ...
        if self.with_mask:
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                mask_results = self._mask_forward(
                    x, mask_rois, semantic_feat=semantic_feat, glbctx_feat=glbctx_feat, relayed_feat=relayed_feat
                )
    ...
    def aug_test(self, img_feats, proposal_list, img_metas, rescale=False):
        ...
        for x, img_meta, semantic_feat, glbctx_feat in zip(img_feats, img_metas, semantic_feats, glbctx_feats):
            ...
            bboxes, scores = self.bbox_head[-1].get_bboxes(
                rois, cls_score, bbox_results["bbox_pred"], img_shape, scale_factor, rescale=False, cfg=None
            )
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                det_segm_results = self.mask_head.get_seg_masks(
                    merged_masks, det_bboxes, det_labels, rcnn_test_cfg, ori_shape, scale_factor=1.0, rescale=False
                )
