    ...
    def __init__(
        ...
        if train_cfg is not None:
            for stage in range(num_stages):
                assert isinstance(self.bbox_sampler[stage], PseudoSampler), "Sparse R-CNN only support `PseudoSampler`"
    ...
    def forward_train(
        ...
        for stage in range(self.num_stages):
            ...
            single_stage_loss = self.bbox_head[stage].loss(
                cls_score.view(-1, cls_score.size(-1)), decode_bbox_pred.view(-1, 4), *bbox_targets, imgs_whwh=imgs_whwh
            )
