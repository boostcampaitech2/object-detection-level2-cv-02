...
class GeneralData(NiceRepr):
            ...
            if k in self._meta_info_fields:
                ...
                if isinstance(ori_value, (torch.Tensor, np.ndarray)):
                    if (ori_value == v).all():
                        ...
                        raise KeyError(
                            f"img_meta_info {k} has been set as " f"{getattr(self, k)} before, which is immutable "
                        )
                elif ori_value == v:
                    continue
                else:
    ...
    def __setattr__(self, name, val):
        if name in ("_meta_info_fields", "_data_fields"):
            if not hasattr(self, name):
                ...
                raise AttributeError(f"{name} has been used as a " f"private attribute, which is immutable. ")
        else:
            if name in self._meta_info_fields:
                raise AttributeError(f"`{name}` is used in meta information," f"which is immutable")
    ...
    def __delattr__(self, item):
        ...
        if item in ("_meta_info_fields", "_data_fields"):
            raise AttributeError(f"{item} has been used as a " f"private attribute, which is immutable. ")

        if item in self._meta_info_fields:
            raise KeyError(f"{item} is used in meta information, " f"which is immutable.")
    ...
    def pop(self, *args):
        ...
        name = args[0]
        if name in self._meta_info_fields:
            raise KeyError(f"{name} is a key in meta information, " f"which is immutable")
