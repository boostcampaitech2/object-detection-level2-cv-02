    ...
    def grid_priors(self, featmap_sizes, device="cuda", with_stride=False):
        ...
        for i in range(self.num_levels):
            priors = self.single_level_grid_priors(
                featmap_sizes[i], level_idx=i, device=device, with_stride=with_stride
            )
            multi_level_priors.append(priors)
        return multi_level_priors

    def single_level_grid_priors(self, featmap_size, level_idx, device="cuda", with_stride=False):
        ...
        shift_x = (torch.arange(0.0, feat_w, device=device) + self.offset) * stride_w
        shift_y = (torch.arange(0.0, feat_h, device=device) + self.offset) * stride_h
        shift_xx, shift_yy = self._meshgrid(shift_x, shift_y)
        if not with_stride:
