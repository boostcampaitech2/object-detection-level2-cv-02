...
class CocoPanopticDataset(CocoDataset):
    ...
    def evaluate_pan_json(self, result_files, outfile_prefix, logger=None, classwise=False):
        ...
        pq_stat = pq_compute_multi_core(matched_annotations_list, gt_folder, pred_folder, self.categories)
    ...
    def evaluate(self, results, metric="PQ", logger=None, jsonfile_prefix=None, classwise=False, **kwargs):
        ...
        if "PQ" in metrics:
            eval_pan_results = self.evaluate_pan_json(result_files, outfile_prefix, logger, classwise)
