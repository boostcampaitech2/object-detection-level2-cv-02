...
class LoadPanopticAnnotations(LoadAnnotations):
    ...
    def __init__(
        self, with_bbox=True, with_label=True, with_mask=True, with_seg=True, file_client_args=dict(backend="disk")
    ):
        if rgb2id is None:
            raise RuntimeError(
        ...
        super(LoadPanopticAnnotations, self).__init__(
            with_bbox, with_label, with_mask, with_seg, True, file_client_args
        )
