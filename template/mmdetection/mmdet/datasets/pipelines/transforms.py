...
class Pad:
    ...
    def __init__(self, size=None, size_divisor=None, pad_to_square=False, pad_val=dict(img=0, masks=0, seg=255)):
        self.size = size
...
class RandomCrop:
    ...
    def __init__(
        self, crop_size, crop_type="absolute", allow_negative_crop=False, recompute_bbox=False, bbox_clip_border=True
    ):
...
class Mosaic:
    ...
    def __init__(self, img_scale=(640, 640), center_ratio_range=(0.5, 1.5), min_bbox_size=0, pad_val=114):
    ...
    def _mosaic_transform(self, results):
        ...
        if len(results["img"].shape) == 3:
            mosaic_img = np.full(
                (int(self.img_scale[0] * 2), int(self.img_scale[1] * 2), 3), self.pad_val, dtype=results["img"].dtype
            )
        else:
            mosaic_img = np.full(
...
class RandomAffine:
    ...
    def __call__(self, results):
        ...
        warp_matrix = translate_matrix @ shear_matrix @ rotation_matrix @ scaling_matrix @ center_matrix
        ...
        for key in results.get("bbox_fields", []):
            ...
            if num_bboxes:
                ...
                # filter bboxes
                valid_index = self.filter_gt_bboxes(bboxes * scaling_ratio, warp_bboxes)
