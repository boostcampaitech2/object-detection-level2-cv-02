...
def test_yolact_head_loss():
    ...
    segm_head = YOLACTSegmHead(
        in_channels=256, num_classes=80, loss_segm=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=1.0)
    )
    mask_head = YOLACTProtonet(
        num_classes=80, in_channels=256, num_protos=32, max_masks_to_train=100, loss_mask_weight=6.125
    )
