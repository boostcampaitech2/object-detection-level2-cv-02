...
def test_corner_head_loss():
    ...
    empty_gt_losses = self.loss(
        tl_heats, br_heats, tl_embs, br_embs, tl_offs, br_offs, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore
    )
    ...
    gt_bboxes = [
        torch.Tensor([[23.6667, 23.8757, 238.6326, 151.8874], [123.6667, 123.8757, 138.6326, 251.8874]]),
    ]
...
def test_corner_head_encode_and_decode_heatmap():
    ...
    targets = self.get_targets(
        gt_bboxes, gt_labels, feat[0].shape, img_metas[0]["pad_shape"], with_corner_emb=self.with_corner_emb
    )
