...
def test_fsaf_head_loss():
    ...
    cfg = dict(
        ...
        anchor_generator=dict(
            type="AnchorGenerator", octave_base_scale=1, scales_per_octave=1, ratios=[1.0], strides=[8, 16, 32, 64, 128]
        ),
    ...
    train_cfg = mmcv.Config(
        dict(
            assigner=dict(type="CenterRegionAssigner", pos_scale=0.2, neg_scale=0.2, min_pos_iof=0.01),
            allowed_border=-1,
            pos_weight=-1,
            debug=False,
        )
