...
from mmdet.models.necks import FPN, YOLOXPAFPN, ChannelMapper, CTResNetNeck, DilatedEncoder, SSDNeck, YOLOV3Neck
...
def test_fpn():
    ...
    fpn_model = FPN(
        in_channels=in_channels, out_channels=out_channels, start_level=1, add_extra_convs=False, num_outs=5
    )
    outs = fpn_model(feats)
    assert len(outs) == fpn_model.num_outs
    ...
    fpn_model = FPN(
        in_channels=in_channels, out_channels=out_channels, add_extra_convs="on_input", start_level=1, num_outs=5
    )
    assert fpn_model.add_extra_convs == "on_input"
    outs = fpn_model(feats)
    assert len(outs) == fpn_model.num_outs
    ...
    fpn_model = FPN(
        in_channels=in_channels, out_channels=out_channels, add_extra_convs="on_lateral", start_level=1, num_outs=5
    )
    ...
    fpn_model = FPN(
        in_channels=in_channels, out_channels=out_channels, add_extra_convs="on_output", start_level=1, num_outs=5
    )
...
def test_channel_mapper():
    ...
    # in_channels
    with pytest.raises(AssertionError):
        channel_mapper = ChannelMapper(in_channels=in_channels[:-1], out_channels=out_channels, kernel_size=kernel_size)
...
def test_ct_resnet_neck():
    ...
    ct_resnet_neck = CTResNetNeck(
        in_channel=in_channels, num_deconv_filters=num_filters, num_deconv_kernels=num_kernels, use_dcn=False
    )
    ...
    if torch.cuda.is_available():
        # test dcn
        ct_resnet_neck = CTResNetNeck(
            in_channel=in_channels, num_deconv_filters=num_filters, num_deconv_kernels=num_kernels
        )
