...
def test_sparse_rcnn_forward():
    ...
    with torch.no_grad():
        ...
        detector.roi_head.simple_test(
            [imgs[0][None, :]], torch.empty((1, 0, 4)), torch.empty((1, 100, 4)), [img_metas[0]], torch.ones((1, 4))
        )
...
def test_faster_rcnn_ohem_forward():
    ...
    losses = detector.roi_head.forward_train(
        feature, img_metas, [torch.empty((0, 5))], gt_bboxes=gt_bboxes, gt_labels=gt_labels
    )
    assert isinstance(losses, dict)
...
def test_two_stage_forward(cfg_file):
    ...
    # Save cost
    if cfg_file in ["seesaw_loss/mask_rcnn_r50_fpn_random_seesaw_loss_normed_mask_mstrain_2x_lvis_v1.py"]:  # noqa: E501
    ...
    with torch.no_grad():
        ...
        detector.roi_head.aug_test(
            features, [torch.cat([x1y1, x2y2], dim=-1), torch.empty((0, 4))], [[img_metas[0]]] * 2
        )
        ...
        if cfg_file not in cascade_models:
            ...
            bboxes, scores = detector.roi_head.simple_test_bboxes(
                feature, [img_metas[0]] * 2, [torch.empty((0, 4)), torch.cat([x1y1, x2y2], dim=-1)], None
            )
...
def _demo_mm_inputs(input_shape=(1, 3, 300, 300), num_items=None, num_classes=10, with_semantic=False):  # yapf: disable
    ...
    model_dict = dict(
        ...
        bbox_head=dict(
            ...
            anchor_generator=dict(
                type="AnchorGenerator", octave_base_scale=4, scales_per_octave=3, ratios=[0.5], strides=[32]
            ),
            bbox_coder=dict(
