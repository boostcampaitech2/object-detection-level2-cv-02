...
def test_csp_darknet_backbone():
    ...
    model = CSPDarknet(arch="P6", widen_factor=1.5, out_indices=range(0, 6), spp_kernal_sizes=(3, 5, 7))
    model.init_weights()
    model.train()
    ...
    model = CSPDarknet(widen_factor=1.0, act_cfg=dict(type="ReLU"), out_indices=range(0, 5))
    model.init_weights()
    model.train()
    ...
    model = CSPDarknet(arch_ovewrite=arch_ovewrite, widen_factor=1.0, out_indices=(0, 1, 2, 3))
    model.init_weights()
    model.train()
