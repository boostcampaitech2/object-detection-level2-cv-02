...
@pytest.mark.parametrize(
    "loss_class", [IoULoss, BoundedIoULoss, GIoULoss, DIoULoss, CIoULoss, MSELoss, L1Loss, SmoothL1Loss, BalancedL1Loss]
)
...
def test_classification_losses(loss_class, input_shape):
    if input_shape[0] == 0 and digit_version(torch.__version__) < digit_version("1.5.0"):
        pytest.skip(f"CELoss in PyTorch {torch.__version__} does not support empty" f"tensor.")
...
def test_loss_with_ignore_index(use_sigmoid):
    ...
    loss_class = CrossEntropyLoss(use_sigmoid=use_sigmoid, use_mask=False, ignore_index=255)
    ...
    loss_with_forward_ignore = loss_class(pred, target, ignore_index=250, reduction_override="sum")
