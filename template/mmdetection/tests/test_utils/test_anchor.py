...
def test_sparse_prior():
    ...
    sparse_prior = mlvl_points.sparse_priors(
        prior_idxs=prior_indexs, featmap_size=featmap_sizes[0], level_idx=0, device="cpu"
    )

    assert not sparse_prior.is_cuda
    assert (sparse_prior == grid_anchors[0][prior_indexs]).all()
    sparse_prior = mlvl_points.sparse_priors(
        prior_idxs=prior_indexs, featmap_size=featmap_sizes[1], level_idx=1, device="cpu"
    )
    ...
    for i in range(len(featmap_sizes)):
        sparse_ssd_anchors = anchor_generator.sparse_priors(
            prior_idxs=prior_indexs, level_idx=i, featmap_size=featmap_sizes[i], device="cpu"
        )
    ...
    if torch.cuda.is_available():
        ...
        sparse_prior = mlvl_points.sparse_priors(
            prior_idxs=prior_indexs, featmap_size=featmap_sizes[0], level_idx=0, device="cuda"
        )
        assert (sparse_prior == grid_anchors[0][prior_indexs]).all()
        sparse_prior = mlvl_points.sparse_priors(
            prior_idxs=prior_indexs, featmap_size=featmap_sizes[1], level_idx=1, device="cuda"
        )
        assert (sparse_prior == grid_anchors[1][prior_indexs]).all()
        ...
        for i in range(len(featmap_sizes)):
            sparse_ssd_anchors = anchor_generator.sparse_priors(
                prior_idxs=prior_indexs, level_idx=i, featmap_size=featmap_sizes[i], device="cuda"
            )
            assert (sparse_ssd_anchors == ssd_anchors[i][prior_indexs]).all()
...
def test_strides():
    ...
    expected_anchors = torch.tensor(
        [[-5.0, -5.0, 5.0, 5.0], [5.0, -5.0, 15.0, 5.0], [-5.0, 5.0, 5.0, 15.0], [5.0, 5.0, 15.0, 15.0]]
    )
    ...
    expected_anchors = torch.tensor(
        [[-5.0, -5.0, 5.0, 5.0], [5.0, -5.0, 15.0, 5.0], [-5.0, 15.0, 5.0, 25.0], [5.0, 15.0, 15.0, 25.0]]
    )
