...
def _create_panoptic_style_json(json_name):
    ...
    annotations = [
        {
            "segments_info": [
                {"id": 1, "category_id": 0, "area": 400, "bbox": [50, 60, 20, 20], "iscrowd": 0},
                {"id": 2, "category_id": 1, "area": 900, "bbox": [100, 120, 30, 30], "iscrowd": 0},
                {"id": 3, "category_id": 2, "iscrowd": 0, "bbox": [1, 189, 612, 285], "area": 70036},
            ],
            "file_name": "fake_name1.jpg",
            "image_id": 0,
        },
        {
            "segments_info": [
                ...
                {"id": 4, "category_id": 1, "area": 900, "bbox": [100, 120, 30, 30], "iscrowd": 1},
                {"id": 5, "category_id": 2, "iscrowd": 0, "bbox": [100, 200, 200, 300], "area": 66666},
                {"id": 6, "category_id": 0, "iscrowd": 0, "bbox": [1, 189, -10, 285], "area": 70036},
            ],
            "file_name": "fake_name2.jpg",
            "image_id": 1,
        },
...
def test_load_panoptic_style_json():
    ...
    dataset = CocoPanopticDataset(
        ann_file=fake_json_file, classes=[cat["name"] for cat in fake_json["categories"]], pipeline=[]
    )
...
def _create_panoptic_gt_annotations(ann_file):
    ...
    annotations = [
        {
            "segments_info": [
                {"id": 1, "category_id": 0, "area": 400, "bbox": [10, 10, 10, 40], "iscrowd": 0},
                {"id": 2, "category_id": 0, "area": 400, "bbox": [30, 10, 10, 40], "iscrowd": 0},
                {"id": 3, "category_id": 1, "iscrowd": 0, "bbox": [50, 10, 10, 5], "area": 50},
                {"id": 4, "category_id": 2, "iscrowd": 0, "bbox": [0, 0, 80, 60], "area": 3950},
            ],
...
def test_panoptic_evaluation():
    ...
    pred_bboxes = np.array(
        [[11, 11, 10, 40], [38, 10, 10, 40], [51, 10, 10, 5]],  # TP IoU=351/449=0.78  # FP  # TP IoU=45/55=0.818
        dtype=np.int64,
    )
    ...
    dataset = CocoPanopticDataset(
        ann_file=ann_file, seg_prefix=tmp_dir.name, classes=[cat["name"] for cat in gt_json["categories"]], pipeline=[]
    )
