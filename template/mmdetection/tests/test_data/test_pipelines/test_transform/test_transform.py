...
def test_random_center_crop_pad():
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="RandomCenterCropPad", crop_size=(511, 511), ratios=(1.0), test_mode=False, test_pad_mode=None
        )
        build_from_cfg(transform, PIPELINES)
...
def test_multi_scale_flip_aug():
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="MultiScaleFlipAug", scale_factor=1.0, img_scale=[(1333, 800)], transforms=[dict(type="Resize")]
        )
        build_from_cfg(transform, PIPELINES)
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="MultiScaleFlipAug", img_scale=[(1333, 800)], flip_direction=1, transforms=[dict(type="Resize")]
        )
        build_from_cfg(transform, PIPELINES)
    ...
    scale_factor_transform = dict(
        type="MultiScaleFlipAug", scale_factor=[0.8, 1.0, 1.2], transforms=[dict(type="Resize", keep_ratio=False)]
    )
