...
def test_shear():
    ...
    results_gt = copy.deepcopy(results)
    img_s = np.array(
        [[1, 6, img_fill_val, img_fill_val], [5, img_fill_val, img_fill_val, img_fill_val]], dtype=np.uint8
    )
    img_s = np.stack([img_s, img_s, img_s], axis=-1)
    results_gt["img"] = img_s
    ...
    policies = [
        [
            ...
            dict(type="Shear", level=8, img_fill_val=img_fill_val, direction="vertical", max_shear_magnitude=1.0),
        ]
    ]
    autoaug = dict(type="AutoAugment", policies=policies)
    autoaug_module = build_from_cfg(autoaug, PIPELINES)
