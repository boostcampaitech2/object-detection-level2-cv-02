...
def test_rotate():
    ...
    img_r = np.stack(
        [np.ones((h, w)) * img_fill_val[0], np.ones((h, w)) * img_fill_val[1], np.ones((h, w)) * img_fill_val[2]],
        axis=-1,
    ).astype(np.uint8)
    img_r[0, 0, :] = 1
    img_r[0, 1, :] = 5
    ...
    policies = [
        [
            ...
            dict(type="Rotate", level=8, max_rotate_angle=90, center=(0), img_fill_val=img_fill_val),
        ]
    ]
    autoaug = dict(type="AutoAugment", policies=policies)
    autoaug_module = build_from_cfg(autoaug, PIPELINES)
