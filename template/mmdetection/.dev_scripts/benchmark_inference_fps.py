...
def parse_args():
    ...
    parser.add_argument(
        "--round-num", type=int, default=1, help="round a number to a given precision in decimal digits"
    )
    parser.add_argument(
        ...
        for model_info in model_infos:
            ...
            try:
                fps = repeat_measure_inference_speed(
                    cfg, checkpoint, args.max_iter, args.log_interval, args.fuse_conv_bn, args.repeat_num
                )
                if args.repeat_num > 1:
                    ...
                    mean_times_pre_image = round(sum(times_pre_image_list) / len(times_pre_image_list), args.round_num)
                    print(
                        f"{cfg_path} "
                    ...
                    result_dict[cfg_path] = dict(
                        fps=round(fps, args.round_num), ms_times_pre_image=round(1000 / fps, args.round_num)
                    )
            except Exception as e:
                print(f"{cfg_path} error: {repr(e)}")
                if args.repeat_num > 1:
                    result_dict[cfg_path] = dict(fps=[0], mean_fps=0, ms_times_pre_image=[0], mean_times_pre_image=0)
                else:
