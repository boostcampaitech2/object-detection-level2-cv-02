...
if __name__ == "__main__":
    ...
    if args.excel:
        ...
        if xlrd is None:
            raise RuntimeError("xlrd is not installed," "Please use “pip install xlrd==1.2.0” to install")
        if xlutils is None:
            raise RuntimeError("xlutils is not installed," "Please use “pip install xlutils==2.0.0” to install")
        ...
        for i, config in enumerate(model_cfgs):
            ...
            if osp.exists(result_path):
                ...
                if osp.exists(osp.join(result_path, ckpt_path)):
                    ...
                    # 3 read metric
                    model_performance = get_final_results(log_json_path, total_epochs, final_results_out)
                    ...
                    if args.excel:
                        if "AR@1000" in model_performance:
                            ...
                            metrics = f'{model_performance["bbox_mAP"]}/' f'{model_performance["segm_mAP"]}'
                        else:
