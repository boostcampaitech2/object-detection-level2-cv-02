...
if __name__ == "__main__":
                ...
                if len(json_list) > 0:
                    ...
                    if config in metric.get("config", {}):
                        ...
                        for record_metric_key in record_metrics:
                            ...
                            new_metric = round(metric["metric"][record_metric_key] * 100, 1)
                            new_metrics[record_metric_key_bk] = new_metric

                        if args.show_all:
                            result_dict[config] = dict(before=record_metrics, after=new_metrics)
                        else:
                            for record_metric_key in record_metrics:
                                old_metric = record_metrics[record_metric_key]
    ...
    if metrics_out:
        ...
        mmcv.dump(result_dict, osp.join(metrics_out, "batch_test_metric_info.json"))
