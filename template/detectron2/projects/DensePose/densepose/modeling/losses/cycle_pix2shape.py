...
class PixToShapeCycleLoss(nn.Module):
    ...
    def forward(
        ...
        for pixel_embeddings, mask_gt in zip(pix_embeds, masks_gt):
            ...
            for mesh_name in mesh_names:
                ...
                sim_matrix = pixel_embeddings_sampled.mm(mesh_vertex_embeddings.T)  # pyre-ignore[16]
    ...
    def fake_value(self, densepose_predictor_outputs: Any, embedder: nn.Module):
        losses = [embedder(mesh_name).sum() * 0 for mesh_name in embedder.mesh_names]  # pyre-ignore[29]
