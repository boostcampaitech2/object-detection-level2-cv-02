...
class DensePoseOutputsVisualizer(object):
    ...
    def visualize(
        ...
        N = S.size(0)
        assert N == I.size(0), "densepose outputs S {} and I {}" " should have equal first dim size".format(
            S.size(), I.size()
        )
        assert N == U.size(0), "densepose outputs S {} and U {}" " should have equal first dim size".format(
            S.size(), U.size()
        )
        assert N == V.size(0), "densepose outputs S {} and V {}" " should have equal first dim size".format(
            S.size(), V.size()
        )
        assert N == len(
            bboxes_xywh
        ...
        for n in range(N):
            ...
            if self.to_visualize == "I":
                ...
                for partId in range(U_or_Vn.shape[0]):
                    vis[segmentation == partId] = U_or_Vn[partId][segmentation == partId].clip(0, 1) * 255
